...
- **Action Button** (`ListEntitiesEventResponse` / `EventResponse`, `button_hold`)
  ...
  **A ringing timer takes the tap ahead of everything except the hold**
  (`em_button.TAP_RING_STOP`). HA discards a timer as it fires, so nothing
  upstream can stop a ring and the wake word is the only acoustic stop —
  weakest in exactly the loud room an alarm rings in. It therefore outranks
  `tap_event` and, critically, **mute**: muting kills the mic and so kills
  the wake word, which used to leave a muted ring with no stop but the cap.
  That is not a hole in the mute rule — the rule is about starting a voice
  TURN, and silencing an alarm is not speech. `mute_state` stops the ring for
  the same reason. Both must call `stop_timer_ring`, never the turn-cancel
  path: a ring holds `voice_lock`, so a tap classified as `CANCEL` set
  `cancel_event` and silenced the burst without ending the ring, leaving a
  SILENT ring holding the voice lock to `timerRingSeconds` — a button that
  looked like it worked and left the device unable to take a turn.
